...
function Courses() {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_FRONTEND_URL}/api/courses`)
      ...
      const userId = localStorage.getItem("id");
      if(userId){
        fetch(`${process.env.REACT_APP_FRONTEND_URL}/api/learning/${userId}`)
          .then((response) => response.json())
          .then((data) => {
            console.log(data);
            let arr = [];
  ...
  function enrollCourse(courseId) {
    if(authToken){
      ...
      axios.post('${process.env.REACT_APP_FRONTEND_URL}/api/learning', enrollRequest)
          .then((response) => {
