...
const Performance = () => {
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_FRONTEND_URL}/api/learning/${userId}`);
  ...
  useEffect(()=>{
    const userId = localStorage.getItem("id");
    fetch(`${process.env.REACT_APP_FRONTEND_URL}/api/assessments/perfomance/${userId}`).then((res)=>res.json()).then((data)=>{setPerfomanceData(data)});
