...
const Course = () => {
  ...
  const handleDuration = () => {
    ...
    if(duration!=0){
      fetch("${process.env.REACT_APP_FRONTEND_URL}/api/progress/update-duration", {
        method: "PUT",
        headers: {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_FRONTEND_URL}/api/progress/${userId}/${courseId}`)
  ...
  useEffect(() => {
    const updateProgress = async () => {
        if (courseId && userId) {
            try {
                const response = await fetch(`${process.env.REACT_APP_FRONTEND_URL}/api/progress/update-progress`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                    },
